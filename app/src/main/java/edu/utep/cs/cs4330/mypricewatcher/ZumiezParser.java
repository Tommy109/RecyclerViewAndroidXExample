...
public class ZumiezParser extends AsyncTask<String, Void, Item> {
    ...
    public ZumiezParser(ItemFetching itemFetching) {
    ...
    protected Item doInBackground(String... param) {
        ...
        try {
            ...
            String name = doc.select("h1.reset-margin").text();
            String price = doc.select("span.regular-price").first().select("span").first().text();
            String imageUrl = doc.select("img.thumb-image").first().attr("src");
